...
const unsigned short crc_table[] = { 	0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48, 0x9dc1, 
...
void startCRC(unsigned short* crc)
{
    *crc = 0xffff;
}

unsigned short updateCRC(unsigned short* crc, char chr)
{
    *crc = (((*crc) >> 8) & 0xff) ^ crc_table[((*crc) ^ chr) & 0xff];
    return ~(*crc);
}
